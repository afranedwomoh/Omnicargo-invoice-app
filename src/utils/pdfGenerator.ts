...
let cachedMark: LoadedImage | null = null
let cachedWordmark: LoadedImage | null = null

const loadImageAsDataUrl = (url: string): Promise<LoadedImage | null> => {
...
// Loads the icon mark and wordmark images used in the PDF header, matching
// the same two-image lockup used in the downloadable/shared PNG version.
// Falls back to a hand-drawn mark + text wordmark (drawLogoMark below) if
// either file is missing.
const getLogoImages = async (): Promise<{ mark: LoadedImage | null; wordmark: LoadedImage | null }> => {
  if (!cachedMark) cachedMark = await loadImageAsDataUrl('/omnicargo-mark.png')
  if (!cachedWordmark) cachedWordmark = await loadImageAsDataUrl('/omnicargo-wordmark.png')
  return { mark: cachedMark, wordmark: cachedWordmark }
}

// Draws a small filled dot to the left of a contact detail line (address,
// phone, email) in the PDF, as a lightweight stand-in for the SVG icons used
// in the PNG version (jsPDF can't render inline SVG).
const drawContactBullet = (doc: jsPDF, x: number, y: number, slate: string) => {
  doc.setFillColor(slate)
  doc.circle(x, y - 1.1, 0.7, 'F')
}
...
const generateLocalDeliveryPDF = async (invoiceData: InvoiceData): Promise<void> => {
  ...
  const { mark, wordmark } = await getLogoImages()
  if (mark && wordmark) {
    const markHeight = 8.5
    const markWidth = markHeight * (mark.width / mark.height)
    const wordmarkHeight = 10
    const wordmarkWidth = wordmarkHeight * (wordmark.width / wordmark.height)
    // Placed well clear of the 14mm-tall banner strip above, so the
    // transparent-background logo images never overlap the colored header.
    doc.addImage(mark.dataUrl, 'PNG', 15, 18, markWidth, markHeight)
    doc.addImage(wordmark.dataUrl, 'PNG', 15 + markWidth + 2, 17, wordmarkWidth, wordmarkHeight)
  } else {
  ...
  if (invoiceData.business.address) {
    drawContactBullet(doc, 27, contactY, slate)
    const addressLines = doc.splitTextToSize(invoiceData.business.address, 87)
...
export const generateInvoicePDF = async (invoiceData: InvoiceData): Promise<void> => {
  ...
  let contactY = 40
  if (invoiceData.business.address) {
    drawContactBullet(doc, 19, contactY, slate)
    const addressLines = doc.splitTextToSize(invoiceData.business.address, 87)
...
// html2canvas snapshots the DOM synchronously relative to when it's called -
// it does not wait for <img> tags inside that DOM to finish loading. Since
// the logo header now uses two separate image files (icon + wordmark)
// instead of one, both must be confirmed loaded first, or they can render
// blank in the captured image even though they display fine in a normal
// page view (where the browser has time to fetch them naturally).
...
export const generateInvoiceImage = async (invoiceData: InvoiceData): Promise<string> => {
  ...
  await preloadImages(['/omnicargo-mark.png', '/omnicargo-wordmark.png'])
