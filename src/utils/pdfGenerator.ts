...
const generateLocalDeliveryPDF = async (invoiceData: InvoiceData): Promise<void> => {
  ...
  if (mark && wordmark) {
    ...
    const wordmarkHeight = 13
    const wordmarkWidth = wordmarkHeight * (wordmark.width / wordmark.height)
    doc.addImage(mark.dataUrl, 'PNG', 15, 9, markWidth, markHeight)
    doc.addImage(wordmark.dataUrl, 'PNG', 15 + markWidth + 2, 8, wordmarkWidth, wordmarkHeight)
  } else {
